...
class Thread extends Eloquent
{
    ...
    protected $fillable = [
        'subject', 'slug', 
        'start_date', 'end_date', 
        'max_participants', 'avatar', 'is_read', // added 'is_read'
    ];
    ...
    */
    ...
    /**
     * Marks the thread as read for a user.
     *
     * @param int $userId
     * @param bool $isRead
     * @return void
     */
    public function markAsReadForUser($userId, $isRead = true)
    {
        try {
            $participant = $this->getParticipantFromUser($userId);
            $participant->update(['is_read' => $isRead]);
        } catch (ModelNotFoundException $e) {
    ...
    /**
     * Checks if the thread is read for a specific user.
     *
     * @param int $userId
     * @return bool
     */
    public function isReadForUser($userId)
    {
        try {
            $participant = $this->getParticipantFromUser($userId);
            return $participant->is_read;
        } catch (ModelNotFoundException $e) {
            return false;
        }
    }
